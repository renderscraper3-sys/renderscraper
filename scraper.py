        ...
        try:
            ...
            async with session.get(url, headers=obtener_headers_dinamicos(), timeout=20) as r:
                if r.status == 200:
                    ...
                    # Extracción combinada de Imagen y Breadcrumbs (Categorías) vía JSON-LD
                        ...
                        try:
                            ...
                            for obj in objetos:
                                 ...
                                 if isinstance(obj, dict) and obj.get("@type") == "Product" and obj.get("image"):
                                     ...
                                     if not imagen_url_original: # Solo si no la tenemos
                                 ...
                                 # Extraer Categorías Divididas
                                 if isinstance(obj, dict) and obj.get("@type") == "BreadcrumbList":
                                     items = obj.get("itemListElement", [])
                                     # Ordenamos por posición para asegurar la jerarquía
                                     nombres = [item.get("name") for item in sorted(items, key=lambda x: int(x.get("position", 0)))]
                                     
                                     # Limpiamos el nodo "Inicio" si existe
                                     if nombres and nombres[0].lower() in ["inicio", "home"]:
                                         nombres = nombres[1:]
                                         
                                     cat1 = nombres[0] if len(nombres) > 0 else None
                                     cat2 = nombres[1] if len(nombres) > 1 else None
                                     cat3 = nombres[2] if len(nombres) > 2 else None
                        except Exception: continue
                            
                    # Fallback para imagen por si no estaba en el JSON
        ...
        if "/art_" in url:
            ...
            if datos:
                ...
                if res_prod.data:
                    ...
                    print(f"✅ Guardado: {datos['nombre'][:30]}... | Cat: {datos['cat1']} > {datos['cat2']} | ${datos['precio_actual']}")
