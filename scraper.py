...
def extraer_datos_producto(session, url):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Lógica corregida para extraer Marca y Categorías del JSON
                ...
                try:
                    ...
                    for obj in objetos:
                        ...
                        if obj.get("@type") == "BreadcrumbList":
                            ...
                            # ESTA ERA LA FALLA: Función para evitar crash al ordenar (texto vs número)
                            def safe_pos(x):
                                try: return int(x.get("position", 99))
                                except: return 99
                                
                            items_sorted = sorted([i for i in items if isinstance(i, dict)], key=safe_pos)
            ...
            # ESCUDO FALLBACK: Por si el JSON falla, saca la categoría del HTML
            if not cat1:
                breadcrumb = soup.find(class_=re.compile(r"breadcrumb|migas", re.I)) or soup.find("ul", class_=re.compile(r"breadcrumb", re.I))
                if breadcrumb:
