                            ...
                            for obj in objetos:
                                if (
                                    ...
                                    if isinstance(img_data, str):
                                        imagen_url_original = img_data
                                    elif isinstance(img_data, list) and img_data:
                                        imagen_url_original = img_data[0]
                                    break
                        except Exception:
                            continue
...
async def guardar_en_db(datos, url_imagen_storage):
    try:
        ...
        res_prod = await db_execute(
            supabase.table("productos_laanonima").upsert(producto_data, on_conflict="ean")
        )
        ...
        if res_prod.data:
            ...
            await db_execute(
                supabase.table("precios_historial").insert(historial_data)
            )

            print(
                f"✅ ¡Éxito de BD! Guardado: {datos['nombre'][:30]}... | "
                f"EAN: {datos['ean']} | ${datos['precio_actual']}"
            )
    ...
    try:
        async with session.get(url_categoria, headers=obtener_headers_dinamicos(), timeout=30) as r:
            ...
            if r.status == 200:
                ...
                urls_productos = [
                    urljoin(url_categoria, a.get("href", "").strip())
                    for a in productos
                    if a.get("href")
                ]
                ...
                if urls_productos:
                    ...
                    if datos:
                        ...
                        if datos.get("imagen_url_original"):
                            ...
                            url_img_storage = await procesar_imagen(
                                session,
